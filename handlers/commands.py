...
async def handle_callback(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data == "menu_tasks":
        ...
        if today_tasks:
            sections.append(f"СЕГОДНЯ — {today.strftime('%d.%m.%Y')}")
        if tomorrow_tasks:
            sections.append(f"ЗАВТРА — {tomorrow.strftime('%d.%m.%Y')}")
        if no_deadline:
        ...
        # Кнопки задач (макс 20)
        buttons = []
        for r in rows[:20]:
            icon = "📅" if r['deadline'] else "○"
            buttons.append([InlineKeyboardButton(
                f"{icon} {r['title'][:35]}",
...
async def handle_task_action(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("taskdate_"):
        ...
        if len(parts) >= 2 and parts[1] == "manual":
            ...
            await query.edit_message_text("Напиши дату:\nФормат: 20.09 или 20.09.2026")
            return
...
async def auto_confirm_date(context: ContextTypes.DEFAULT_TYPE):
    ...
    await context.bot.send_message(
        chat_id=chat_id,
        text=f"✅ Срок задачи автоматически установлен: {date_str}"
    )
...
async def handle_confirm_date(update: Update, context: ContextTypes.DEFAULT_TYPE):
    ...
    if data.startswith("confirmdate_yes_"):
        ...
        try:
            ...
            pass
        await query.edit_message_text(f"✅ Срок задачи установлен: {date}")
        return
